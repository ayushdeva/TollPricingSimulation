...
class LinkManager(Simulator):
	...
	def __init__(self,a,b):
		...
		self.nToll = np.array([0 for i in xrange(11)])
		self.flag = 0 #to choose each arm once atleast - keeps count of no. of iterations

	def calcRoot(self,nj,n):
		term1 = float(2)*math.log(n)/float(nj)
		return math.sqrt(term1)

	def chooseToll(self):
		# print("Choosing toll with epsilon as ", self.epsilon)
		if(self.flag<=10):
			self.toll = self.flag*10
			self.flag+=1
		else : 
			self.flag+=1
			tollVal,preMax = 0,0
			for i in xrange(11):
				tmp = self.qVal[i] + self.calcRoot(self.nToll[i],self.flag)
				if(tmp>preMax):
					tollVal = i
			self.toll = tollVal*10 

	def updateState(self,rev,newTime):
		self.time = newTime
		self.qVal[self.toll/10] = self.qVal[self.toll/10]*self.nToll[self.toll/10]+rev
		self.nToll[self.toll/10] += 1
		self.qVal[self.toll/10] /= self.nToll[self.toll/10]
		self.chooseToll()
